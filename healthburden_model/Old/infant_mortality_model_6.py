...
def safrica_lat(input):
    return -40.0 <= input <= 40.0

def safrica_long(input):
    return -20.0 <= input <= 53.0

saf_con2 = iris.Constraint(latitude = safrica_lat, longitude = safrica_long)
    
pop_afr = pop_2000.extract(saf_con2)
#iris.save(pop_afr, '/nfs/a321/earsch/CHAMNHA/input_data/world_pop_mf_afr_subset.nc')

country_totals = iris.load_cube('/nfs/a321/earsch/CHAMNHA/input_data/pop/country_under5_totalpopmf_2000.nc')
cs = pop_afr.coord('latitude').coord_system
country_totals.coord('longitude').coord_system = cs
country_totals.coord('latitude').coord_system = cs
country_totals = country_totals.regrid(pop_afr, iris.analysis.Linear())

fraction_under5 = pop_afr /country_totals
#iris.save(fraction_under5, '/nfs/a321/earsch/CHAMNHA/input_data/pop/fraction_under5.nc')


#mortality rate
mor_2000 = iris.load_cube('/nfs/a321/earsch/CHAMNHA/input_data/mortality/morrate_under5_noshocks_2000_mf.nc')
mor_2000.units = 'unknown'
...
#import grid size information
gs = iris.load_cube('/nfs/a321/earsch/CHAMNHA/input_data/gridarea.nc')
gs.convert_units('km2')

pop_2000 = pop_2000 * gs # convert from pop/km2 to pop/gridcell

mor_2000 = mor_2000.regrid(pop_2000, iris.analysis.Linear())

totmor = mor_2000 * pop_2000 # total under 5 mortality per gridcell
daily_mor = totmor / 365

#iris.save(daily_mor, '/nfs/a321/earsch/CHAMNHA/input_data/mortality/daily_under5_mortality_2000_mf_noshocks.nc')
