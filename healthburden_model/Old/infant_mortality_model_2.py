...
#%% modify threshold by koppen
##source: http://koeppen-geiger.vu-wien.ac.at/shifts.htm
kop = iris.load_cube('/nfs/a321/earsch/CHAMNHA/input_data/af_koppen_1976_2000.nc')

ghana = kop.interpolate((['latitude', 8.15], ['longitude', -0.93]), iris.analysis.Linear()).data
kenya = kop.interpolate((['latitude', -1.2], ['longitude', 36.8]), iris.analysis.Linear()).data

kop.data  = np.where(kop.data < 20, 1, kop.data) #ghana
kop.data  = np.where(kop.data > 1, 2, kop.data) #kenya

kop = kop.regrid(tas[0][0], iris.analysis.Linear())
kop.data = ma.masked_array(kop.data, mask = tas[0][0].data.mask)
...
tdif_list = iris.list.CubeList()
for cube in tas:
    tdif = calc_tdif(tas[0], thres)
    tdif_list.append(tdif)
