...
mor_better = pd.read_csv('/nfs/a321/earsch/CHAMNHA/input_data/mortality/future/vis_ratedeath_better_both_04.csv')
mor_worse = pd.read_csv('/nfs/a321/earsch/CHAMNHA/input_data/mortality/future/vis_ratedeath_worse_both_04.csv')
...
#deaths per 100,000
present_day_morrate = mor_2019 / pop2019 * 100000
...
def distr_dat(in_table, scenario_name):
    ...
    for y in years:
        ...
        #save_name = scenario_name + '_' + str(y) + '_04_ratemor_mf.nc'
        #iris.save(mor_year_frac, save_path + save_name)
        
        output.append(mor_year)
    return output
...
#%%
print(np.nanmean(present_day_morrate.data))
...
plt.plot(years, mmor, label = 'Reference')
plt.plot(years, mmorb, label = 'Better')
plt.plot(years, mmorw, label = 'Worse')

plt.scatter(2019, np.nanmean(present_day_morrate.data), label = 'actual 2019')
plt.xlabel('Year')
