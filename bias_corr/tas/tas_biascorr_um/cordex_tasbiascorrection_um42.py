...
def extract_years(obs, mod_list, obs_years, myears ):
    ...
    return cru_years, tas_his, tas_future
...
def bias_corr(obs, mod_list, area, ls, obs_years, myears, path):
    ...
    #convert to celsius
    cru_years, tas_his, tas_future = extract_years(safrica_cru, safrica_mods, obs_years, myears)
...
#obs_years = [1971, 2000]
obs_years = [1997, 2006]
myears = [1997, 2006] #cp4 future same years as present 
...
filenames = glob.glob(path + '*cp4*grid*rcp85*.nc')
tas = iris.cube.CubeList()
tas_constraint = iris.Constraint(cube_func=lambda cube: cube.var_name == 'd03236')
for file in filenames:
    x = iris.load_cube(file,  tas_constraint)
    if 'cp4' in file:
        mod = 'cp4'
    else:
        mod = 'p25'
    x.add_aux_coord(iris.coords.AuxCoord('um', long_name = 'gcm'))
    x.add_aux_coord(iris.coords.AuxCoord(mod, long_name = 'model'))
    x.add_aux_coord(iris.coords.AuxCoord('rcp85', long_name = 'sim'))
    tas.append(x)
    
#tas his
path = '/nfs/a321/earsch/Tanga/Data/CORDEX/Processed/tas/'
filenames = glob.glob(path + '*cp4*grid*his*.nc')
tas_his = iris.cube.CubeList()
tas_constraint = iris.Constraint(cube_func=lambda cube: cube.var_name == 'd03236')
for file in filenames:
    x = iris.load_cube(file,  tas_constraint)
    ...
    x.add_aux_coord(iris.coords.AuxCoord('um', long_name = 'gcm'))
    x.add_aux_coord(iris.coords.AuxCoord(mod, long_name = 'model'))
    x.add_aux_coord(iris.coords.AuxCoord('historical', long_name = 'sim'))
    tas_his.append(x)
